...
/// Voici un sujet du TP du 14/03/2022 en algo.
/// Ce TP requiert une bonne compréhension du cours, et etre à l'aise avec l'algo en général.
/// Si une matière doit etre négligée, choisis l'élec.
...
/// Ici la fonction à appeler dans le "main()" pour voir l'exercice 1.
int main1() {

    // On crée un tableau de 10 entiers et on appelle la fonction "charger()" qui remplit le tableau avec les valeurs
    // dans le fichier dont le nom est le premier paramètre
    int tablCharge[10];
    charger("fichier",tablCharge);

    // Impression rapide de mon tableau (pour vérifier que la fonction précédente ait bien fonctionné
    for(int i = 0; i < 10; i++) {
        printf("%d\n", tablCharge[i]); // fun fact: le "f" de printf veut dire "format".
    }

    // Je demande la valeur à remplacer et je la stocke
    ...
    // Encore un affichage du tableau, afin de voir les valeurs modifiées et j'indique le nombre de cases modifiées
    for(int i = 0; i < 10; i++) {
...
// Aaaah une belle fonction. Pour ceux qui ne l'auraient pas compris, l'avantage de la récursivité est sa compacité.
// On peut faireun code bien plus compact pour une meme opération.
int modifier(int tablEntiers[10], int tablTaille, int v, int compte) {
    ...
    if (tablEntiers[tablTaille-1] == v) {
        tablEntiers[tablTaille-1] = 0;
        compte++;
    }

    // On relance la fonction avec les mêmes parametres sauf au niveau de "tabTaille" qui baisse de un à chaque répétition
...
void charger(char* fpName, int tablCharge[10]) {    // Note le "char* fpName", ca permet de gérer un tableau de taille variable
    ...
    // On n'oublie pas, bien sur, de fermer le fichier
    fclose(fp);
}
...
// Donc faut vite aller aider ton binôme!
...
// Quoi?! Une structure sauvage apparaît? ouf... c'est pas un tableau dynamique
...
void creerJeu(t_carteS tabl[52]) {
    ...
    // Boucle for qui se répete 52 fois
    for(int i = 0; i < 52; i++) {
...
void afficherCarte(t_carteS* carte) {
    ...
    // Le voici!!! Le misérable switch case!
    ...
    switch (carte->suit) {              // Ici on définit la variable à considérer
        case 1:                             // Et on considére les états de la variable. Ici, si elle vaut 1 on entre dans le "case"
    ...
    // Un deuxième switch case pour considérer la valeur de la carte
    switch (carte->value) {
        case 11:
            printf("Valet\n");
            break;
...
// Si tu lis ce commentaire, t'es vrmt trop chaud <3
...
// Oh crotte... la difficulté monte. Mais on est ing non?
typedef struct pile{        // Structure pile qui va pouvoir contenir une pile de carte
    ...
    int sommet;             // L'indice de sortie et d'entrée qui indique où rajouter ou enlever des éléments
}t_pile;
...
void afficherPile(t_pile* pile) {   // Beaucoup de "pile" en une ligne...
    ...
    // Impression de "???" pour chaque carte qui n'est pas "retournée" (comme au solitaire...)
...
void depilerEmpiler(t_pile* p1, t_pile* p2) {
    ...
    // Le if qui fache:
        ...
        // ... si la valeur de la carte sur la pile1 est supérieure à celle de la pile2 OU si la pile2 est vide...
    ...
    if(p1->sommet != 0 && p2->sommet < p2->tailleMax && (p1->tab[p1->sommet-1].value > p2->tab[p2->sommet-1].value || p2->sommet == 0)) {
        ...
        // En théorie, un appel de "ajouterCarte devrait etre possible, mais je l'ai réimplémenté ici
    ...
    // Et si le if n'est pas validé, on affiche "impossible"
    else {
        printf("Impossible!!\n\n");
    }
}
