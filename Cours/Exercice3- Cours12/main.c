...
// Agrandis la fenetre de sorte à ce que la file tienne sur une ligne pour mieux comprendre comment le tout fonctionne
...
int main() {
    ...
    for(int i = 0; i < 50; i++) {       // J'initialise toutes les valeurs du tableau à '_' pour mieux visualiser, mais aussi pour garantir le fonctionnement de "estPleine"
        fileStruct->fileElements[i] = '_';
    }
...
int estVide(struct file* fileStruct) {
    ...
    // Je vérifie aussi que l'element à l'indice de retrait est vide (j'ai donc forcément une file vide)
    if (fileStruct->indiceAjout == fileStruct->indiceRetrait && fileStruct->fileElements[fileStruct->indiceRetrait] == '_')
        return 1;
...
int estPleine(struct file* fileStruct) {
    // Ici, je vérifie encore que les deux indices sont aux meme endroit, mais cette fois ci, je vérifie que l'élément à l'indice d'ajout n'est pas vide (et donc que ma file est pleine)
    if (fileStruct->indiceAjout == fileStruct->indiceRetrait && fileStruct->fileElements[fileStruct->indiceAjout] != '_')
        return 1;
